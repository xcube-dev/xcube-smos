...
@click.option(
    "--debug",
    is_flag=True,
    help="Output verbose debugging information." " NOT IMPLEMENTED YET.",
)
@click.option(
    "--traceback",
    is_flag=True,
    help="Output stack traceback on errors." " NOT IMPLEMENTED YET.",
)
@click.pass_context
...
@click.option(
    ...
    nargs=1,
    metavar="<key>",
    help="S3 access key identifier if source protocol is" f' "{S3_PROTOCOL}".',
)
@click.option(
    "--s3-secret",
    nargs=1,
    metavar="<secret>",
    help="S3 secret access key if source protocol is" f' "{S3_PROTOCOL}".',
)
@click.option(
    "--s3-anon",
    is_flag=True,
    default=None,
    help="Force anonymous S3 access if source protocol is" f' "{S3_PROTOCOL}".',
)
def create(
    ctx,
    index_path,
    ...
    if source_protocol == S3_PROTOCOL:
        ...
        source_path = (
            f"{source_path}/{s3_bucket}"
            if source_path and s3_bucket
            else source_path
            if source_path
            else s3_bucket
        )
        source_storage_options = {
            k: v for k, v in s3_storage_options.items() if v is not None
        }
    else:
        source_storage_options = {}
    if not source_path:
...
@click.option("--prefix", "prefix_path", metavar="<path>", help="Source prefix path")
@click.option("--force", is_flag=True, help="Do not skip existing indexes.")
@click.option("--dry-run", is_flag=True, help="Do not create any indexes.")
@click.pass_context
def sync(ctx, index_path, prefix_path, force, dry_run):
    """Synchronize a NetCDF Kerchunk index."""
    from xcube_smos.nckcindex.nckcindex import NcKcIndex

    # click.echo(f"Debug is {'on' if ctx.obj['DEBUG'] else 'off'}")
    nc_kc_index = NcKcIndex.open(index_path=index_path)
    num_files, problems = nc_kc_index.sync(
        prefix=prefix_path, force=force, dry_run=dry_run
    )
    print(
        f"{num_files} file(s) synchronized"
