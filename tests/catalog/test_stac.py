...
from xcube_smos.constants import DEFAULT_STAC_URL
...
class SmosStacCatalogTest(unittest.TestCase):
    def test_fetch_features(self):
        features = fetch_features(
            product_prefix="SM_OPER_MIR_SMUDP2",
        ...
        features = fetch_features(
            product_prefix="SM_OPER_MIR_OSUDP2",
            date_range=("2023-05-01", "2023-05-02"),
            bbox=(0, 40, 20, 60),
            limit=20
        )
    ...
    def test_stac(self):
        params = create_request_params(date_range=("2023-05-01", "2023-05-30"),
                                       bbox=(0, 40, 20, 60),
                                       limit=20)

        response = requests.get(DEFAULT_STAC_URL + "/items", params)
        ...
        for features in features:
            product_id = features["id"]
            if product_id.startswith("SM_OPER_MIR_OSUDP2"):
                os_count += 1
            if product_id.startswith("SM_OPER_MIR_SMUDP2"):
                sm_count += 1
